...
1. Agent (or user via coordinator) writes/updates a `.md` file in `~/dev/workspace/memory/` or `~/dev/workspace/MEMORY.md`
2. Files are plain markdown — no special format required
3. Changes are NOT automatically indexed (the index must be triggered)

### Index Path

1. `openclaw memory index` scans `~/dev/workspace/memory/` and `~/dev/workspace/MEMORY.md`
...
- [ ] File count matches: `ls ~/dev/workspace/memory/*.md | wc -l` matches indexed file count (minus 1 for MEMORY.md at workspace root)
