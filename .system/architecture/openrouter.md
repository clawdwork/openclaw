...
| `DeepSeek-V4-Pro`   | `openrouter/deepseek/deepseek-v4-pro`    | **1M**  | $0.435 | $0.870 | ✅         | ✅                       | ✅ cache-read $0.036/M (~0.08x), cache-write free. **1.6T params, 49B active.** Used by `quality-critic` agent (replaced `openai/gpt-5.4`).                                                                                                               |
| `DeepSeek-V4-Flash` | `openrouter/deepseek/deepseek-v4-flash`  | **1M**  | $0.140 | $0.280 | ✅         | ✅                       | ✅ cache-read $0.028/M (~0.20x), cache-write free. 284B params, 13B active. Cheap-tier alternative.                                                                                                                                                       |
...
**Orchestrator (main agent)** is configured to use `openrouter/moonshotai/kimi-k2.6` with `thinkingDefault: medium` as of 2026-04-25 — replaces the prior default `google/gemini-3-flash-preview`. Originally deployed at `high`; downshifted to `medium` since the Coordinator's role is routing decisions and synthesis (not deep reasoning), and high-effort thinking on every routing call inflates output tokens by 25–30% for negligible quality lift. Verified live: `openclaw agent --agent main --message "..."` returns clean response routed through OpenRouter. **DeepSeek V4 Pro was evaluated and rejected for Coordinator** because it is text-only (no image input) and lacks native web search — both are required for the user-facing conversation surface. V4 Pro remains the right choice for text-only critical-path agents (Critic, Prod Coder, Planner).

**Quality-critic agent** is configured to use `openrouter/deepseek/deepseek-v4-pro` with `thinkingDefault: high` as of 2026-04-25 — replaces the prior `openai/gpt-5.4`. Cost rationale: V4 Pro at $0.435/M in / $0.87/M out vs GPT-5.4 at $2.50/$15 = roughly **5× cheaper input / 17× cheaper output**, with 1M context (vs 266K), 1.6T params, native reasoning, tools, and structured-output support — well-suited to the critic's SOTA-review role.
