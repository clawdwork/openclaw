...
OpenClaw includes a built-in **WebChat** channel (`ws://127.0.0.1:9173`). Each agent can be accessed via WebChat by selecting the agent. For a custom webapp or iOS app, hit the gateway HTTP API directly — the gateway IS the backend. No additional server required.
